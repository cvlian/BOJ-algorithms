...
class mergeTree:
    ...
    def __init__(self, n):
        self.size = 2**(math.ceil(math.log(n, 2)) + 1)
        self.v = [[] for _ in range(self.size)]
    ...
    def fill(self, p):
        ...
        self.v[p] = sorted(self.fill(2*p)+self.fill(2*p+1))
        return self.v[p]

    def cal(self, p, L, R, nodeL, nodeR, k):
        if R < nodeL or nodeR < L:
            return 0
        if L <= nodeL and nodeR <= R:
            return self.v[p]
        ...
        m = (nodeL + nodeR)//2
        return self.cal(2*p, L, R, nodeL, m, k) + self.cal(2*p + 1, L, R, m + 1, nodeR, k)
